@extends('layouts.app')

@section('css')
  <link href="{{ asset('css/show.css') }}" rel="stylesheet">
@endsection

@section('content')
  <div class="container">
    <h1 class='pagetitle'>レビュー詳細ページ</h1>
      <div class="card">
        <div class="card-body d-flex">
          <section class='review-main'>
            <h2 class='h2'>本のタイトル</h2>
            <!--ReviewController の return view('show', compact('review'));で渡された変数$review-->
            <p class='h2 mb20'>{{ $review->title }}</p>
            <h2 class='h2'>レビュー本文</h2>
            <p>{{ $review->body }}</p>
          </section>  
          <aside class='review-image'>
            @if(!empty($review->image))
              <!--レビュー投稿時にサムネイル画像が投稿されていた場合は、storage/app/public/imagesの中のサムネイル画像を出し、-->
              <!--サムネイル画像を登録していない場合はダミー画像が出力されます。-->
              <img class='book-image' src="{{ asset('storage/images/'.$review->image) }}">
            @else
              <img class='book-image' src="{{ asset('images/dummy.png') }}">
            @endif
          </aside>
        </div>
        <!--トップページへ戻る-->
        <a href="{{ route('index') }}" class='btn btn-info btn-back mb20'>一覧へ戻る</a>
      </div>
  </div>
@endsection